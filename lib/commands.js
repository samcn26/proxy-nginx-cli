...
function addSite(domain, target, options = {}, cwd = process.cwd()) {
  ...
  if (options.cert) {
    output += `\n${certProject(domain, cwd, runCompose)}`;
  }
...
function createExample(options = {}) {
  ...
  runCompose(['up', '-d', '--build', '--force-recreate', 'proxy-nginx'], { cwd: proxyDir });
...
function upProject(cwd = process.cwd(), runCompose = defaultComposeRunner) {
  assertProject(cwd);
  runCompose(['up', '-d', '--build', '--force-recreate', 'proxy-nginx'], { cwd });
...
function statusProject(cwd = process.cwd(), runCompose = defaultComposeRunner) {
  assertProject(cwd);
  runCompose(['ps'], { cwd });

  const sites = listSites(cwd);
  const networks = readProxyNetworks(fs.readFileSync(path.join(cwd, 'docker-compose.yml'), 'utf8'));
  const certificates = listCertificates(cwd);

  return [
    'Proxy nginx project status.',
    formatList('Sites', sites),
    formatList('Networks', networks),
    formatList('Certificates', certificates),
  ].join('\n');
}
...
function backupSelfSignedCert(domain, cwd, now) {
  ...
  try {
    ...
    moveCertDirAsRoot(certsDir, domain, path.basename(backupDir));
  }
}

function moveCertDirAsRoot(certsDir, domain, backupName) {
  execFileSync('docker', [
    'run',
    '--rm',
    '-v',
...
function parseTarget(target, options) {
  if (target) {
    ...
    return {
      ...
      host: normalizeUpstreamHost(url.hostname),
      port: url.port,
    };
  }
  ...
  return {
    protocol: 'http',
    host: normalizeUpstreamHost(options.host),
...
function readProxyNetworks(compose) {
  const proxyBlock = getProxyServiceBlock(compose);
  const match = proxyBlock.match(/\n    networks:\n((?:      - .+\n)+)/);
  if (!match) {
    return [];
  }
...
function writeProxyNetworks(compose, networks) {
  ...
  const proxyBlock = getProxyServiceBlock(compose);
  const start = compose.indexOf('  proxy-nginx:\n');
  const end = compose.indexOf('\n  certbot:', start);
  let nextProxyBlock = proxyBlock.replace(/\n    networks:\n(?:      - .+\n)+/g, '');

  if (uniqueNetworks.length > 0) {
    nextProxyBlock = `${nextProxyBlock.replace(/\n+$/, '')}
    networks:
      - default
${uniqueNetworks.map((name) => `      - ${name}`).join('\n')}
`;
  }

  let nextCompose = `${compose.slice(0, start)}${nextProxyBlock}${compose.slice(end)}`;
...
function getProxyServiceBlock(compose) {
  const start = compose.indexOf('  proxy-nginx:\n');
  const end = compose.indexOf('\n  certbot:', start);
  if (start === -1 || end === -1) {
    throw new Error('docker-compose.yml must include proxy-nginx and certbot services.');
  }
...
function listSites(cwd) {
  ...
  return fs
    ...
    .filter((filename) => filename.endsWith('.conf.template'))
    .filter((filename) => filename !== '00-unmatched-host.conf.template')
    .map((filename) => filename.replace(/\.conf\.template$/, ''))
    .sort();
}

function listCertificates(cwd) {
  ...
  return fs
    ...
    .filter((filename) => fs.statSync(path.join(renewalDir, filename)).size > 0)
    .map((filename) => filename.replace(/\.conf$/, ''))
    .sort();
}
