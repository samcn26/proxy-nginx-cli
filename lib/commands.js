...
function createExample(options = {}) {
  ...
  const isPortOpen = options.isPortOpen || isPortInUseSync;
...
function certProject(domain, cwd = process.cwd(), runCompose = defaultComposeRunner) {
  if (!domain) {
    throw new Error('Domain is required.');
  }
...
function createComposeRunner(execFile = execFileSync) {
  return (args, options = {}) => {
    return execFile('docker-compose', args, {
      stdio: 'inherit',
      ...options,
    });
  };
...
function stopPortListeners(port) {
  try {
    execFileSync('sh', [
      '-c',
      `lsof -tiTCP:${Number(port)} -sTCP:LISTEN | xargs -r kill`,
    ], {
      stdio: 'ignore',
    });
  } catch {
...
server.listen(port, '127.0.0.1', () => {
  console.log(\`proxy-nginx-cli example backend listening on http://127.0.0.1:\${port}\`);
});
`;
}
